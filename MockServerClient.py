...
class MockServerClient(object):
    ...
    def _call(self, command, data=None):
        ...
        return requests.put("{}/{}".format(base_url, command), data=data)

    def reset(self):
        self.expectations = []
        self._call("reset")

    def mock_callback(self, requestMatcher, requestHandler):
        ws = WebSocketClient(self.ip, self.port)
        self.websockets[ws.clientId] = ws
        ws.clientHander = requestMatcher
        ws.requestHandler = requestHandler
        t = threading.Thread(target=ws.run, args=())
        t.start()

    def callback_close(self, id):
        self.websockets[id].on_close()
...
def request_for_callback(client_id, request, timing=None):
    return _non_null_options_to_dict(
        _Option("httpRequest", request),
        _Option("times", (timing or _Timing()).for_expectation()),
        _Option("httpResponseObjectCallback", client_id.getIdInfo())
    )
